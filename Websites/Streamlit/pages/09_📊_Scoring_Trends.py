...
def compute_offensive(off_stats: pd.DataFrame, roster: pd.DataFrame, season_filter: str) -> pd.DataFrame:
    ...
    # For offensive view: QB gets passing TDs, others get rush + rec TDs
    df["total_td"] = df.apply(lambda r: r.pass_td if r.position=="QB" else r.rush_td + r.rec_td, axis=1)
    pos_map = {"QB":"QB", "RB":"RB", "FB":"RB", "WR":"WR", "TE":"TE"}
...
with col2:
    ...
    with offense_tab:
        ...
        styled_table = display_table.style.map(style_heatmap_offense, subset=["QB (passing)", "WR", "TE", "RB"])
