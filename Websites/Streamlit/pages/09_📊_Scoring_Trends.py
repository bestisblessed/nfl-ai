...
st.set_page_config(page_title="NFL Touchdown Trends", layout="wide")
    ...
    st.markdown(f"""
        <div style='text-align: center;'>
            <div style='font-size: 3.1rem; font-weight: 800; padding-bottom: 0.5rem;'>
                NFL Touchdown Trends
            </div>
            <div style='color: #7f8c8d; font-size: 1rem; margin-top: 0; line-height: 1.2;'>
                Offensive and defensive touchdown analysis by position
            </div>
        </div>
        """,
...
def compute_defensive(def_stats: pd.DataFrame, roster: pd.DataFrame, season_filter: str) -> pd.DataFrame:
    """Compute touchdowns allowed by defense (original functionality)"""
    # Parse season filter
    if '-' in season_filter:
...
def compute_offensive(off_stats: pd.DataFrame, roster: pd.DataFrame, season_filter: str) -> pd.DataFrame:
    """Compute touchdowns scored by offense"""
    # Parse season filter
    if '-' in season_filter:
        # Range of seasons (e.g., "2023-2025")
        start_year, end_year = map(int, season_filter.split('-'))
...
def display_offense_analysis(table: pd.DataFrame):
    """Display offensive touchdown analysis"""
    ...
    # Style the dataframe with heatmap colors (green for offensive - higher is better)
    def style_heatmap_offense(val):
        if isinstance(val, (int, float)) and val > 0:
            # Create green gradient heatmap for offense (higher is better)
            intensity = min(val / 10, 1)  # Normalize to 0-1 scale
            # Use light green to dark green gradient
            red = int(240 - (190 * intensity))    # Start at light green (240), go to dark green (50)
            green = 255  # Keep green at max
            blue = int(240 - (190 * intensity))   # Start at light green (240), go to dark green (50)
    ...
    # Most productive by position section - 2x2 layout
    st.write('####')
        ...
        with col:
            ...
            for idx, (_, row) in enumerate(top5.iterrows(), 1):
                ...
                <div class="productive-item">
                    <span class="productive-team">{idx}. {team}</span>
                    <span class="productive-tds">{tds} TDs</span>
                </div>
                """
            ...
            <div class="productive-card">
                <div class="productive-title">{title}</div>
                {items_html}
            </div>
            """
...
def display_defense_analysis(table: pd.DataFrame):
    """Display defensive touchdown analysis (original functionality)"""
...
with col2:
    # Create tabs for Offense and Defense
    offense_tab, defense_tab = st.tabs(["🏈 Offense", "🛡️ Defense"])
    ...
    with offense_tab:
        ...
        display_offense_analysis(table)
    ...
    with defense_tab:
        ...
        display_defense_analysis(table)
