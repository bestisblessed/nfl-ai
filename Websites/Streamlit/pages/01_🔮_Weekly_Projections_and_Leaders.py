...
@st.cache_data
def load_game_times() -> Dict[str, Dict[str, str]]:
    """Load game times from odds JSON files to sort games by start time."""
    games_with_times: Dict[str, Dict[str, str]] = {}
    try:
...
def load_games_for_week(week: int) -> pd.DataFrame:
    ...
    if os.path.exists(games_file):
        try:
            ...
            week_games = games_df[(games_df["season"] == 2025) & (games_df["week"] == week)]
...
@st.cache_data
def load_upcoming_games() -> Tuple[Dict[Tuple[str, str], str], pd.DataFrame]:
...
def add_questionable_tag(name: str, questionable_players: set[str]) -> str:
    if pd.isna(name):
        return name
...
@st.cache_data
def load_projections_data() -> Tuple[List[str], List[int]]:
...
@st.cache_data
def get_week_projections(week_num: int) -> pd.DataFrame | None:
    ...
    try:
        ...
        st.error(f"Error loading week {week_num} data: {str(exc)}")
        return None


def create_leaderboard_from_projections(
    df: pd.DataFrame,
    prop_type: str,
    title: str,
...
def create_styled_dataframe(leaderboard: Dict[str, pd.DataFrame]) -> pd.io.formats.style.Styler:
...
st.set_page_config(
    page_title="Weekly Projections and Leaders",
    page_icon="🏈",
    layout="wide",
)
...
st.markdown(
    ...
    <div style='text-align: center;'>
        <div style='font-size: 3.1rem; font-weight: 800; padding-bottom: 0.5rem;'>
            Weekly Projections and Leaders
        </div>
        <div style='color: #7f8c8d; font-size: 1rem; margin-top: 0; line-height: 1.2;'>
            Weekly player projections, leaderboards, and matchup-specific breakdowns
        </div>
    </div>
    """,
...
_games_mapping, upcoming_games_df = load_upcoming_games()
_game_times = load_game_times()
week_games_df = load_games_for_week(int(selected_week))

matchups: List[Tuple[str, str]] = []
if not week_games_df.empty and {"home_team", "away_team"}.issubset(week_games_df.columns):
    week_games_df = week_games_df.copy()
    if {"date", "gametime"}.issubset(week_games_df.columns):
    ...
    for _, row in upcoming_games_df.iterrows():
        ...
        time_info = None
        for key, value in _game_times.items():
    ...
    games_list.sort(key=lambda entry: entry["sort_key"])
...
matchup_options: List[Tuple[str, str]] = [("All Games", "All Games")]
...
selected_matchup_option = persistent_selectbox(
    ...
    page=PAGE_KEY_PREFIX,
    widget="matchup",
    default=matchup_options[0],
    container=st.sidebar,
    format_func=option_display_label,
)
selected_matchup_key = None
selected_matchup_label = None
if selected_matchup_option and selected_matchup_option[0] != "All Games":
    selected_matchup_key = selected_matchup_option[0]
    selected_matchup_label = selected_matchup_option[1]
...
show_leaders = selected_matchup_key is None

if show_leaders:
    current_week = selected_week_display

    leaderboards: List[Dict[str, pd.DataFrame]] = []
        ...
        with tab:
            if boards:
                if len(boards) > 1:
                    ...
                    for col, leaderboard in zip(cols, boards):
                        with col:
                            df = leaderboard["dataframe"]
    ...
    if filtered_df.empty:
        ...
        with qb_cols[0]:
            qb_passing_data = filtered_df[
                (filtered_df["position"] == "QB") & (filtered_df["prop_type"] == "Passing Yards")
            ].copy()

            if not qb_passing_data.empty:
                st.markdown(
                    '<h4 style="text-align: center; font-size: 1.1em; margin-bottom: 0.5em;">QB Passing Yards</h4>',
                    unsafe_allow_html=True,
                )
                display_df = qb_passing_data.sort_values("pred_yards", ascending=False)
                st.dataframe(
                    display_df[["full_name", "team", "pred_yards"]].rename(
                        columns={"full_name": "Player", "team": "Team", "pred_yards": "Projected Yards"}
                    ),
                    width="stretch",
                    hide_index=True,
                )
        ...
        with qb_cols[1]:
            qb_rushing_data = filtered_df[
                (filtered_df["position"] == "QB") & (filtered_df["prop_type"] == "Rushing Yards")
            ].copy()

            if not qb_rushing_data.empty:
                st.markdown(
                    '<h4 style="text-align: center; font-size: 1.1em; margin-bottom: 0.5em;">QB Rushing Yards</h4>',
                    unsafe_allow_html=True,
                )
        ...
        with rb_cols[0]:
            rb_rushing_data = filtered_df[
                (filtered_df["position"] == "RB") & (filtered_df["prop_type"] == "Rushing Yards")
            ].copy()

            if not rb_rushing_data.empty:
                st.markdown(
                    '<h4 style="text-align: center; font-size: 1.1em; margin-bottom: 0.5em;">RB Rushing Yards</h4>',
                    unsafe_allow_html=True,
                )
        ...
        with rb_cols[1]:
            rb_receiving_data = filtered_df[
                (filtered_df["position"] == "RB") & (filtered_df["prop_type"] == "Receiving Yards")
            ].copy()

            if not rb_receiving_data.empty:
                st.markdown(
                    '<h4 style="text-align: center; font-size: 1.1em; margin-bottom: 0.5em;">RB Receiving Yards</h4>',
                    unsafe_allow_html=True,
                )
        ...
        with wr_te_cols[0]:
            wr_receiving_data = filtered_df[
                (filtered_df["position"] == "WR") & (filtered_df["prop_type"] == "Receiving Yards")
            ].copy()

            if not wr_receiving_data.empty:
                st.markdown(
                    '<h4 style="text-align: center; font-size: 1.1em; margin-bottom: 0.5em;">WR Receiving Yards</h4>',
                    unsafe_allow_html=True,
                )
        ...
        with wr_te_cols[1]:
            te_receiving_data = filtered_df[
                (filtered_df["position"] == "TE") & (filtered_df["prop_type"] == "Receiving Yards")
            ].copy()

            if not te_receiving_data.empty:
                st.markdown(
                    '<h4 style="text-align: center; font-size: 1.1em; margin-bottom: 0.5em;">TE Receiving Yards</h4>',
                    unsafe_allow_html=True,
                )
