...
PAGE_KEY_PREFIX = "weekly_projections_and_leaders"
...
def load_upcoming_games() -> Tuple[Dict[Tuple[str, str], str], pd.DataFrame]:
    ...
    if os.path.exists(games_file):
        ...
        games_mapping: Dict[Tuple[str, str], str] = {}
...
@st.cache_data
def load_projections_data() -> Tuple[List[str] | None, List[int]]:
...
matchups: List[Tuple[str | None, str]] = []
        ...
        games_list.append(
            {
                ...
                "sort_key": (
                    time_info["date"] if time_info else "",
                    time_info["time"] if time_info else "",
                ),
            }
        )
...
leaders_option = (None, "All Games · Weekly Leaders")
matchup_options = [leaders_option] + matchups
...
is_leaders_view = selected_matchup_key is None
...
with st.sidebar:
    ...
    if is_leaders_view:
        pdf_filename = f"week{selected_week}_leader_tables.pdf"
