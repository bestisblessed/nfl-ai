...
def load_value_data(week: int) -> Tuple[pd.DataFrame, Optional[pd.DataFrame]]:
    ...
    if {"predicted_yards", "best_point"}.issubset(value_df.columns):
        best_points = value_df["best_point"].replace(0, np.nan)
        value_df["edge_pct"] = (
            (value_df["predicted_yards"] - value_df["best_point"]) / best_points
        ) * 100
    ...
    if os.path.exists(edges_path):
        ...
        if edges_df is not None and {"predicted_yards", "best_point"}.issubset(
            edges_df.columns
        ):
            best_points = edges_df["best_point"].replace(0, np.nan)
            edges_df["edge_pct"] = (
                (edges_df["predicted_yards"] - edges_df["best_point"]) / best_points
            ) * 100
...
prop_type_labels = sorted(
    value_opportunities["prop_type"].dropna().astype(str).unique().tolist()
)
tab_labels: List[str]
if prop_type_labels:
    tab_labels = ["All Props"] + prop_type_labels
else:
    tab_labels = ["All Props"]

PROP_TAB_WIDGET_KEY = "value_prop_tabs"
PROP_TAB_MEMORY_KEY = "value_prop_tabs_last_index"

if PROP_TAB_MEMORY_KEY not in st.session_state:
    st.session_state[PROP_TAB_MEMORY_KEY] = 0

if st.session_state[PROP_TAB_MEMORY_KEY] >= len(tab_labels):
    st.session_state[PROP_TAB_MEMORY_KEY] = 0

st.session_state.setdefault(PROP_TAB_WIDGET_KEY, st.session_state[PROP_TAB_MEMORY_KEY])

if st.session_state[PROP_TAB_WIDGET_KEY] != st.session_state[PROP_TAB_MEMORY_KEY]:
    st.session_state[PROP_TAB_WIDGET_KEY] = st.session_state[PROP_TAB_MEMORY_KEY]

prop_tabs = st.tabs(tab_labels, key=PROP_TAB_WIDGET_KEY)

selected_tab_index = st.session_state.get(PROP_TAB_WIDGET_KEY, 0)
selected_tab_index = int(np.clip(selected_tab_index, 0, len(tab_labels) - 1))
st.session_state[PROP_TAB_MEMORY_KEY] = selected_tab_index

for tab_label, tab in zip(tab_labels, prop_tabs):
    with tab:
        subset = (
            filtered_opportunities
            if tab_label == "All Props"
            else filtered_opportunities[
                filtered_opportunities["prop_type"].astype(str) == tab_label
            ]
        )

        if subset.empty:
            st.info("No value plays match the current filters for this prop type.")
            continue

        ordered_subset = subset.sort_values(by="ev_per_1", ascending=False).copy()
        ordered_subset["rank"] = range(1, len(ordered_subset) + 1)

        display_df = ordered_subset[
            [
                "rank",
                "player",
                "position",
    ...
    if value_edges is not None and not value_edges.empty:
        ...
        for tab, prop_type in zip(prop_tabs, prop_names):
            with tab:
                ...
                leaderboard["Edge %"] = leaderboard["Edge %"].round(1)
