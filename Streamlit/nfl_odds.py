    ...
    for file in files:
        ...
        if os.path.isfile(file_path):
            ...
            if file_timestamp and file_timestamp > end_time_dt:
                ...
                os.remove(file_path)
directory = 'data/odds/'  
end_time = input('Input end date like "20240929_1259": ')  
...
### Splitting Columns ###
file_path = 'data/odds/nfl_odds_movements.csv'  # Update with your file path
...
# nfl_odds_data['time_before'] = nfl_odds_data['file1'].apply(lambda f: datetime.strptime(f.split('_')[-1].replace('.json', ''), '%Y%m%d_%H%M') if len(f.split('_')[-1]) == 12 else None)
# nfl_odds_data['time_after'] = nfl_odds_data['file2'].apply(lambda f: datetime.strptime(f.split('_')[-1].replace('.json', ''), '%Y%m%d_%H%M') if len(f.split('_')[-1]) == 12 else None)
...
# nfl_odds_data['time_before'] = nfl_odds_data['file1'].apply(lambda f: datetime.strptime(f.split('_')[-1].replace('.json', '').zfill(4), '%Y%m%d_%H%M') if len(f.split('_')[-1]) == 4 else None)
# nfl_odds_data['time_after'] = nfl_odds_data['file2'].apply(lambda f: datetime.strptime(f.split('_')[-1].replace('.json', '').zfill(4), '%Y%m%d_%H%M') if len(f.split('_')[-1]) == 4 else None)
# nfl_odds_data['time_before'] = nfl_odds_data['time_before'].apply(lambda dt: dt.strftime('%b %d %-I:%M%p') if pd.notnull(dt) else None)
# nfl_odds_data['time_after'] = nfl_odds_data['time_after'].apply(lambda dt: dt.strftime('%b %d %-I:%M%p') if pd.notnull(dt) else None)
def extract_timestamp(filename):
    try:
...
nfl_odds_data['time_before'] = nfl_odds_data['time_before'].apply(lambda dt: dt.strftime('%b %d %-I:%M%p') if pd.notnull(dt) else None)
nfl_odds_data['time_after'] = nfl_odds_data['time_after'].apply(lambda dt: dt.strftime('%b %d %-I:%M%p') if pd.notnull(dt) else None)
nfl_odds_data = nfl_odds_data.drop(columns=['odds_before', 'odds_after'])
print(nfl_odds_data[['team_1', 'team_2', 'team1_odds_before', 'team2_odds_before', 'team1_odds_after', 'team2_odds_after']].head())
nfl_odds_data.to_csv(file_path, index=False)  # Save without index


### Saving Circa Movements ###
