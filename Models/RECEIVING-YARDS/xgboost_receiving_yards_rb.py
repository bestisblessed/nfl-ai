...
print("=== XGBoost RB Receiving Yards Prediction with Current Rosters ===")
...
# ---------------------------
def create_game_rb_table(game_data, team1, team2, game_num):
    ...
    def create_team_table(ax, team_data, team_name, team_color):
        ...
        if team_data.empty:
            ax.text(0.5, 0.5, f'No {team_name} RBs found', 
    ...
    # Save the figure
    plt.tight_layout()
    filename = f"{output_dir}/game_{game_num:02d}_{team1}_vs_{team2}_RB_predictions.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white', pad_inches=0.3)
    plt.close()
    return filename

def create_game_rb_table_cleaned(game_data, team1, team2, game_num):
    ...
    filename = f"{output_dir}/game_{game_num:02d}_{team1}_vs_{team2}_RB_predictions_cleaned.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='#FFFFFF', 
                edgecolor='none', pad_inches=0.4, transparent=False)
    plt.close()
    return filename

def create_game_rb_table_text(game_data, team1, team2, game_num):
    ...
    filename = f"{output_dir}/game_{game_num:02d}_{team1}_vs_{team2}_RB_predictions.txt"
    ...
    with open(filename, 'w') as f:
        ...
        if team1_table_data:
            ...
            team1_table = f"No {team1} RBs found"
        ...
        if team2_table_data:
            ...
            team2_table = f"No {team2} RBs found"
        ...
        summary_lines = [
            ...
            "GAME SUMMARY",
            "-" * 15,
            f"Total RBs: {len(team1_data) + len(team2_data)} ({len(team1_data)} {team1}, {len(team2_data)} {team2})"
        ]
        
        if len(team1_data) + len(team2_data) > 0:
            top_rb = game_data.loc[game_data['pred_rec_yards'].idxmax()]
            summary_lines.append(f"Top Projection: {top_rb['player_name']} ({top_rb['team']}) - {top_rb['pred_rec_yards']:.1f} yards")
...
# ---------------------------
# 6) Create RB game visualization tables
# ---------------------------
# Filter for RBs only (position already included from roster data)
rb_players = upcoming_players[upcoming_players['position'] == 'RB'].copy()

if not rb_players.empty:
    print(f"\nCreating game visualization tables for {len(rb_players)} RBs...")
    
    # Get unique games (both team vs opp and opp vs team represent the same game)
    games = []
    processed_games = set()
    
    for _, row in rb_players[['team', 'opp']].drop_duplicates().iterrows():
    ...
    for i, (team1, team2) in enumerate(games, 1):
        ...
        game_rbs = rb_players[
            ((rb_players['team'] == team1) & (rb_players['opp'] == team2)) |
            ((rb_players['team'] == team2) & (rb_players['opp'] == team1))
        ].copy()
        ...
        if not game_rbs.empty:
            ...
            filename = create_game_rb_table(game_rbs, team1, team2, i)
            created_files.append(filename)
            print(f"  Created: {filename} ({len(game_rbs)} RBs)")
            
            # Create cleaned visualization table (only name and yards)
            filename_cleaned = create_game_rb_table_cleaned(game_rbs, team1, team2, i)
            created_files.append(filename_cleaned)
            print(f"  Created: {filename_cleaned} ({len(game_rbs)} RBs)")
            
            # Create text-based tabulated table
            filename_text = create_game_rb_table_text(game_rbs, team1, team2, i)
            created_files.append(filename_text)
            print(f"  Created: {filename_text} ({len(game_rbs)} RBs)")
    ...
    print("No RBs found in predictions data")
