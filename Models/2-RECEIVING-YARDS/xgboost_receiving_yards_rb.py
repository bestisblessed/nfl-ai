...
print("=== XGBoost RB Receiving Yards Prediction ===")
...
# Last-3 average receiving yards
...
missing_mask = rb_data[feature_cols].isnull().any(axis=1)
if missing_mask.any():
    missing_names = rb_data.loc[missing_mask, "full_name"].tolist()
    print(f"Filled defaults for {len(missing_names)} RBs without full history: {missing_names}")

rb_data.loc[:, feature_cols] = rb_data[feature_cols].fillna(0)
rb_data["has_history"] = ~missing_mask

print(f"Prepared {len(rb_data)} RBs for predictions (including {missing_mask.sum()} without full history)")
...
predictions[output_cols].to_csv(f"{output_dir}/final_week{week_num}_RB_rec_yards_report.csv", index=False)
...
for i, (team1, team2) in enumerate(unique_games, 1):
    ...
    if not game_rbs.empty:
        game_rbs = game_rbs.sort_values('pred_rec_yards', ascending=False)
        
        # Create full PNG table
        team1_data = game_rbs[game_rbs['team'] == team1].sort_values('pred_rec_yards', ascending=False)
        team2_data = game_rbs[game_rbs['team'] == team2].sort_values('pred_rec_yards', ascending=False)
        ...
        fig.suptitle(f'Week {week_num} 2025: {team1} vs {team2} - RB Receiving Yards Predictions', 
        ...
        fig.suptitle(f'{team1} vs {team2} | Week {week_num} 2025 | RB Projected Receiving Yards', 
        ...
        with open(filename_text, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write(f"{team1} vs {team2} | Week {week_num} 2025 | RB Projected Receiving Yards\n")
            f.write("=" * 80 + "\n\n")
            ...
            if len(game_rbs) > 0:
                ...
                f.write(f"Top Projection: {top_rb['full_name']} ({top_rb['team']}) - {top_rb['pred_rec_yards']:.1f} yards\n")
            f.write("=" * 80 + "\n")
        ...
        # Print to terminal
        print(f"\n{'='*80}")
        print(f"{team1} vs {team2} | Week {week_num} 2025 | RB Projected Receiving Yards")
        print(f"{'='*80}")
        for line1, line2 in zip(team1_lines, team2_lines):
