...
print("=== XGBoost WR Receiving Yards Prediction ===")
...
if len(sys.argv) > 1:
    ...
    print("Usage: python xgboost_receiving_yards_wr_simple.py <week_number>")
    sys.exit(1)
...
hist = hist.dropna(subset=["rec_yards"])
...
for w in windows:
    hist[f"targets_l{w}"] = hist.groupby("player_id")["targets"].transform(
        lambda s: s.shift(1).rolling(w, min_periods=1).mean())
    hist[f"receptions_l{w}"] = hist.groupby("player_id")["receptions"].transform(
        lambda s: s.shift(1).rolling(w, min_periods=1).mean())
    hist[f"yards_l{w}"] = hist.groupby("player_id")["rec_yards"].transform(
        lambda s: s.shift(1).rolling(w, min_periods=1).mean())
    # robust to outliers
    hist[f"yards_median_l{w}"] = hist.groupby("player_id")["rec_yards"].transform(
        lambda s: s.shift(1).rolling(w, min_periods=1).median())

# Simple additional baselines
# Last-3 average receiving yards (limited recency)
hist["rec_l3_avg"] = hist.groupby("player_id")["rec_yards"].transform(
    lambda s: s.shift(1).rolling(3, min_periods=1).mean())

# Prior-season per-game average and career median
prev_season = hist["season"].max() - 1
prev_season_means = (
    hist[hist["season"] == prev_season]
    .groupby("player_id")["rec_yards"].mean()
    .rename("rec_prev_season_pg")
    .reset_index()
)
career_median = (
    hist.groupby("player_id")["rec_yards"].median()
    .rename("rec_career_median")
    .reset_index()
)
...
# Feature columns
feature_cols = ([f"targets_l{w}" for w in windows] + 
               [f"receptions_l{w}" for w in windows] + 
               [f"yards_l{w}" for w in windows] +
               [f"yards_median_l{w}" for w in windows] +
               ["rec_l3_avg", "rec_prev_season_pg", "rec_career_median"])

# Prepare training data
train = hist.dropna(subset=feature_cols + ["rec_yards"])
...
# Get current rosters for WRs only
...
wr_data = wr_rosters.merge(player_features, on="player_id", how="left")

# Allow rookies/new players with limited history by backfilling missing features
missing_mask = wr_data[feature_cols].isnull().any(axis=1)
if missing_mask.any():
    missing_names = wr_data.loc[missing_mask, "full_name"].tolist()
    print(f"Filled defaults for {len(missing_names)} WRs without full history: {missing_names}")

wr_data.loc[:, feature_cols] = wr_data[feature_cols].fillna(0)
wr_data["has_history"] = ~missing_mask

print(f"Prepared {len(wr_data)} WRs for predictions (including {missing_mask.sum()} without full history)")

# Attach to upcoming games
games = upcoming[["team", "opp"]].drop_duplicates()
predictions = games.merge(wr_data, on="team", how="inner")

# Make predictions
predictions["pred_rec_yards"] = model.predict(predictions[feature_cols])
...
for i, (team1, team2) in enumerate(unique_games, 1):
    ...
    if not game_wrs.empty:
        ...
        team1_data = game_wrs[game_wrs['team'] == team1].sort_values('pred_rec_yards', ascending=False)
        team2_data = game_wrs[game_wrs['team'] == team2].sort_values('pred_rec_yards', ascending=False)
        ...
        if not team1_data.empty:
            table_data1 = [[row['full_name'], f"{row['pred_rec_yards']:.1f}", 
                           f"{row['targets_l5']:.1f}", f"{row['receptions_l5']:.1f}", f"{row['yards_l5']:.1f}"] 
            ...
            table1 = ax1.table(cellText=table_data1, colLabels=['Player', 'Pred Yards', 'Targets L5', 'Rec L5', 'Yards L5'],
        ...
        if not team2_data.empty:
            table_data2 = [[row['full_name'], f"{row['pred_rec_yards']:.1f}", 
                           f"{row['targets_l5']:.1f}", f"{row['receptions_l5']:.1f}", f"{row['yards_l5']:.1f}"] 
            ...
            table2 = ax2.table(cellText=table_data2, colLabels=['Player', 'Pred Yards', 'Targets L5', 'Rec L5', 'Yards L5'],
                               cellLoc='center', loc='center', bbox=[0.05, 0.05, 0.9, 0.8])
        ...
        plt.tight_layout()
        filename_full = f"{output_dir}/game_{i:02d}_{team1}_vs_{team2}_WR_predictions.png"
        ...
        ax1.axis('tight'); ax1.axis('off')
        if not team1_data.empty:
            clean_data1 = [[row['full_name'], f"{row['pred_rec_yards']:.1f}"] for _, row in team1_data.iterrows()]
        ...
        ax2.axis('tight'); ax2.axis('off')  
        if not team2_data.empty:
            clean_data2 = [[row['full_name'], f"{row['pred_rec_yards']:.1f}"] for _, row in team2_data.iterrows()]
        ...
        plt.tight_layout()
        filename_clean = f"{output_dir}/game_{i:02d}_{team1}_vs_{team2}_WR_predictions_cleaned.png"
        plt.savefig(filename_clean, dpi=300, bbox_inches='tight', facecolor='#FFFFFF', pad_inches=0.4)
        plt.close()
        created_files.append(filename_clean)
        
        # Create text table
        filename_text = f"{output_dir}/game_{i:02d}_{team1}_vs_{team2}_WR_predictions.txt"
        ...
        with open(filename_text, 'w') as f:
            ...
            team1_table_data = [[row['full_name'], f"{row['pred_rec_yards']:.1f}"] for _, row in team1_data.iterrows()]
            team2_table_data = [[row['full_name'], f"{row['pred_rec_yards']:.1f}"] for _, row in team2_data.iterrows()]
            
            team1_table = tabulate(team1_table_data, headers=[f"{team1} Player", "Pred Yards"], 
                                 tablefmt='outline', stralign='left') if team1_table_data else f"No {team1} WRs"
            team2_table = tabulate(team2_table_data, headers=[f"{team2} Player", "Pred Yards"], 
                                 tablefmt='outline', stralign='left') if team2_table_data else f"No {team2} WRs"
            ...
            f.write("-" * 15 + "\n")
            f.write(f"Total WRs: {len(game_wrs)} ({len(team1_data)} {team1}, {len(team2_data)} {team2})\n")
            if len(game_wrs) > 0:
                top_wr = game_wrs.iloc[0]
                f.write(f"Top Projection: {top_wr['full_name']} ({top_wr['team']}) - {top_wr['pred_rec_yards']:.1f} yards\n")
            f.write("=" * 80 + "\n")
        ...
        print(f"  Created game {i:02d}: {len(game_wrs)} WRs")

print(f"\n{len(predictions)} WR predictions saved")
