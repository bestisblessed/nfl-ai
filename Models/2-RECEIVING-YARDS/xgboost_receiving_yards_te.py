...
print("=== XGBoost TE Receiving Yards Prediction ===")
...
if len(sys.argv) > 1:
    ...
    print("Usage: python xgboost_receiving_yards_te_simple.py <week_number>")
    sys.exit(1)
...
# Get current rosters for TEs only
...
predictions = games.merge(te_data, on="team", how="inner")
...
predictions[output_cols].to_csv(f"{output_dir}/final_week{week_num}_TE_rec_yards_report.csv", index=False)
...
for i, (team1, team2) in enumerate(unique_games, 1):
    ...
    if not game_tes.empty:
        ...
        team1_data = game_tes[game_tes['team'] == team1].sort_values('pred_rec_yards', ascending=False)
        team2_data = game_tes[game_tes['team'] == team2].sort_values('pred_rec_yards', ascending=False)
        ...
        plt.tight_layout()
        filename_full = f"{output_dir}/game_{i:02d}_{team1}_vs_{team2}_TE_predictions.png"
        ...
        plt.tight_layout()
        filename_clean = f"{output_dir}/game_{i:02d}_{team1}_vs_{team2}_TE_predictions_cleaned.png"
        plt.savefig(filename_clean, dpi=300, bbox_inches='tight', facecolor='#FFFFFF', pad_inches=0.4)
        plt.close()
        created_files.append(filename_clean)
        
        # Create text table
        filename_text = f"{output_dir}/game_{i:02d}_{team1}_vs_{team2}_TE_predictions.txt"
        ...
        with open(filename_text, 'w') as f:
            ...
            team1_table = tabulate(team1_table_data, headers=[f"{team1} Player", "Pred Yards"], 
                                 tablefmt='outline', stralign='left') if team1_table_data else f"No {team1} TEs"
            team2_table = tabulate(team2_table_data, headers=[f"{team2} Player", "Pred Yards"], 
                                 tablefmt='outline', stralign='left') if team2_table_data else f"No {team2} TEs"
            ...
            f.write("-" * 15 + "\n")
            f.write(f"Total TEs: {len(game_tes)} ({len(team1_data)} {team1}, {len(team2_data)} {team2})\n")
            if len(game_tes) > 0:
                top_te = game_tes.iloc[0]
                f.write(f"Top Projection: {top_te['full_name']} ({top_te['team']}) - {top_te['pred_rec_yards']:.1f} yards\n")
            f.write("=" * 80 + "\n")
        ...
        print(f"  Created game {i:02d}: {len(game_tes)} TEs")

print(f"\n{len(predictions)} TE predictions saved")
