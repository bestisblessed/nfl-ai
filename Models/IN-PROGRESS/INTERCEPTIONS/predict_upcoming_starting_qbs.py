...
MODEL_PATH = '/Users/td/Code/nfl-ai/Models/IN-PROGRESS/INTERCEPTIONS/logreg_model.pkl'
...
def main():
    ...
    # Print full table of all QBs (using tabulate if available)
    try:
        from tabulate import tabulate
        print('Interception probabilities for all QBs:\n')
        print(tabulate(output, headers='keys', tablefmt='github', showindex=False, floatfmt='.3f'))
    except ImportError:
        print('Interception probabilities for all QBs:')
        print(output)

    # Save full table in predictions/ with week number in filename
    predictions_dir = "/Users/td/Code/nfl-ai/Models/IN-PROGRESS/INTERCEPTIONS/predictions"
    os.makedirs(predictions_dir, exist_ok=True)
    out_path = os.path.join(predictions_dir, f"upcoming_qb_interception_probs_week_{predicted_week}.csv")
