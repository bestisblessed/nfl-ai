"""Logistic regression touchdown model based on Pro Football Reference game logs."""
...
def upcoming_predictions(
    ...
    return upcoming.sort_values("touchdown_probability", ascending=False)
...
def main(upcoming_path: str | None = None) -> None:
    ...
    print("\n=== Upcoming Week Touchdown Probabilities (Logistic Regression) ===")
    for team, group in predictions.groupby("team"):
        ...
        for _, row in top_players.iterrows():
            ...
            print(
                f"  {row['player']:<25s} {prob:>6.2%}  odds: {odds:>7.0f}"
            )

    output_path = Path(__file__).resolve().parents[0] / "logistic_regression_predictions.csv"
