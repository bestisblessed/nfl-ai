...
# QB specific changes
print("=== XGBoost QB Passing Yards Prediction ===")
...
hist = hist.dropna(subset=["pass_yards"])
...
predictions[output_cols].to_csv(f"{output_dir}/final_week{week_num}_QB_pass_yards_report.csv", index=False)
...
for i, (team1, team2) in enumerate(unique_games, 1):
    # Get QBs from both teams for this game
    game_qbs = predictions[
        ((predictions['team'] == team1) & (predictions['opp'] == team2)) |
        ((predictions['team'] == team2) & (predictions['opp'] == team1))
    ].copy()
    
    if not game_qbs.empty:
        game_qbs = game_qbs.sort_values('pred_pass_yards', ascending=False)
        
        # Create full PNG table
        team1_data = game_qbs[game_qbs['team'] == team1].sort_values('pred_pass_yards', ascending=False)
        team2_data = game_qbs[game_qbs['team'] == team2].sort_values('pred_pass_yards', ascending=False)
        ...
        fig.suptitle(f'Week {week_num} 2025: {team1} vs {team2} - QB Passing Yards Predictions', 
        ...
        with open(filename_text, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write(f"{team1} vs {team2} | Week {week_num} 2025 | QB Projected Passing Yards\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write("-" * 15 + "\n")
            f.write(f"Total QBs: {len(game_qbs)} ({len(team1_data)} {team1}, {len(team2_data)} {team2})\n")
            if len(game_qbs) > 0:
                top_qb = game_qbs.iloc[0]
                f.write(f"Top Projection: {top_qb['full_name']} ({top_qb['team']}) - {top_qb['pred_pass_yards']:.1f} yards\n")
            f.write("=" * 80 + "\n")
        ...
        # Print to terminal
        print(f"\n{'='*80}")
        print(f"{team1} vs {team2} | Week {week_num} 2025 | QB Projected Passing Yards")
        print(f"{'='*80}")
        for line1, line2 in zip(team1_lines, team2_lines):
            print(f"{line1.ljust(team1_width)}    {line2}")
        
        print(f"  Created game {i:02d}: {len(game_qbs)} QBs")
