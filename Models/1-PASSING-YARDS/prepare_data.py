...
keep = ["player_id","player_name","team","opp","season","week","pass_attempts","completions","pass_yards"]
df = df[keep].copy()

# Ensure numeric types & drop rows without label
for c in ["pass_attempts","completions","pass_yards","season","week"]:
    df[c] = pd.to_numeric(df[c], errors="coerce")
df = df.dropna(subset=["pass_yards"])
